...
## 2026-09-24 — Buyer messages: direction read from `isInterlocutor`, and a repository import bug found on the way

**Decision:** Revisited plan B2 to close part of what "Not yet verified" (`PROJECT_STATUS.md`) called out: this session's network egress still could not reach `api.allegro.pl` or run a real Sandbox request (no credentials in this environment either), but its web search could reach `developer.allegro.pl`'s own pages and `allegro/allegro-api`'s GitHub issues, which the first attempt at B2 could not. `map_message` now reads a message's direction from `author.isInterlocutor` (confirmed there, by two independent pages: `false` is the authenticated seller, `true` the other party) and only falls back to comparing `author.login` against the stored seller login when that field is absent or not a bool. The confirmed message shape, the full endpoint list, and the `createdAt`/`lastMessageDateTime` write-time-not-publish-time risk (`allegro-api` issue #13907) are written into `INTEGRATIONS.md`, "Buyer messages".

While getting the test suite running to check this, `AfterSalesRepository` (plan B4) turned out to have the same bug already found and fixed once in `OrderRepository` (`PROJECT_STATUS.md`, "Not yet verified"): a method named `list` bound in the class body before `for_order`'s own `-> list[tuple[...]]` return annotation was evaluated, so Python resolved `list` to that method instead of the builtin and the whole backend failed to import (`TypeError: 'function' object is not subscriptable`). Fixed the same way: moved `for_order` above `list`. `PROJECT_STATUS.md` had flagged this as worth checking broadly; it was.

**Rationale:** A field Allegro provides for exactly this purpose is a better signal than comparing logins — it holds even for a thread read before an account's login was ever stored, and does not depend on a stale `account_login` staying correct. The repository bug blocked every backend test from collecting at all, which the "Not yet verified" note's own suggestion to check for it elsewhere would have caught; run before, it would have failed the same way `OrderRepository`'s did.

**Consequences:** Still not verified against a real response - see the doubts on sort order and timestamps in `INTEGRATIONS.md`. The messaging scope's name is still not confirmed from a primary source. This did not touch the model or the API surface, so no migration and no `alembic upgrade head` is needed.

## 2026-09-24 — Returns and claims: fixed a beta-header bug, and surfaced Allegro's new 7-day automatic refund without guessing at it

**Decision:** Revisited plan B4 the same way as B2 above, with the same later-session web search reaching `developer.allegro.pl` and `allegro/allegro-api`'s GitHub issues. Two results: `AllegroClient.fetch_issues` (`GET /sale/issues`) was sending the public `Accept` header where every resource under that path needs the beta one, confirmed by the same announcement (`allegro/allegro-api` issue #11711) that introduced it - fixed, along with a test that had locked in the wrong header. Separately, confirmed from Allegro's own help pages and news: since 2026-09-01, Allegro automatically refunds a return from the seller's funds if the seller has not decided within 7 days of *receiving* the parcel back, a different and shorter clock than the 14-days-from-declaration figure this queue already shows. Not implemented: Allegro's customer-returns resource has no field for when the parcel was received, only `createdAt` and `status`, confirmed by the specification and by a real third-party integration's own mapping agreeing with it independently.

**Rationale:** The Accept header fix follows the same evidence standard already used for messaging and the returns endpoint itself (`INTEGRATIONS.md` already had the beta header right for customer returns; this closed the gap for issues) - confirmed by the same primary announcement, not a guess. The refund-window finding is left undone rather than approximated: this queue's own founding rule (`DECISIONS.md`, "Returns and claims: a read-only queue with deadlines") is that every figure here is either Allegro's own or a flagged guess, verified once real access exists, and inventing a "first seen DELIVERED" timestamp to start a 7-day clock would be a schema change built on an assumption about how promptly this queue's own sync happens to run - a materially different kind of guess from restating a number the owner already gave.

**Consequences:** The return deadline shown here can now land after Allegro's own automatic refund on a return whose parcel comes back quickly, since the two clocks start at different moments; this needs the owner's decision on whether and how to track a receipt moment, and is flagged in `INTEGRATIONS.md`, "Returns and claims", as the next thing to raise with them. Nothing here has still run against a real account.
