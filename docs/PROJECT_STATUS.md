...
**Narrowed 2026-09-24** (the same session, and the same route to `developer.allegro.pl`, that narrowed B2 below): `GET /sale/issues` needs the beta `Accept` header, which the client was not sending — fixed, see `DECISIONS.md`. Also surfaced, not yet acted on: Allegro has automatically refunded returns from the seller's funds since 2026-09-01 when the seller has not decided within 7 days of *receiving* the parcel, a different clock than the 14-days-from-declaration figure this queue shows, and Allegro's API gives no field to read a receipt date from. Left for the owner to decide (`INTEGRATIONS.md`).
